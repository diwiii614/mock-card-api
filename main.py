...
app = FastAPI(
    ...
    version="5.0.0",
    description=(
        "Card servicing for one verified cardholder and their single card. Call "
        "identify_customer with the registered mobile number or customer ID: it "
        "sends a one time code and returns a verification_id starting with SES. "
        "Then call verify_otp with that verification_id and the code the customer "
        "reads back; it returns the card_id and the card state, and opens a "
        "session lasting 15 minutes. Every other operation needs the "
        "verification_id and returns 401 until this has been done.\n\n"
        "This holds one customer's own account data only. It does not hold the "
        "bank's policies: fees, interest rates, how EMI works, delivery "
        "timelines, the maximum a limit may be set to, what a feature controls, "
        "dispute windows, and general explanations of why a payment was refused "
        "all come from the policy lookup skill, not from here."
    ),
)
...
def _fail(status: int, code: str, message: str) -> HTTPException:
    """Every failure returns a stable code and a factual message.

    Deliberately no instructions: a real bank API has many consumers and does
    not tell any of them what to call next. Recovery guidance belongs in the
    tool definition and the agent's own operating instructions, which is where
    it now lives. The code is the contract; the message is for a human reading
    a log.
    """
...
def _card(card_id: str) -> dict:
    ...
    if not card:
        raise _fail(404, "CARD_UNKNOWN", "No card exists with that card_id.")
    return card
...
def _session(verification_id: str) -> dict:
    ...
    if not given:
        raise _fail(401, "VERIFICATION_ID_MISSING",
                    "No verification_id was supplied.")
    entry = VERIFICATIONS.get(given)
    if not entry:
        raise _fail(401, "VERIFICATION_ID_NOT_ISSUED",
                    f"'{given[:40]}' was not issued by this service. Every "
                    f"verification_id is generated by identify_customer and cannot be "
                    f"derived from a mobile number or any other value.")
    if entry.get("expired"):
        raise _fail(401, "VERIFICATION_EXPIRED",
                    f"This session expired after {VERIFICATION_MINUTES} minutes.")
    if not entry["verified"]:
        raise _fail(401, "VERIFICATION_INCOMPLETE",
                    "This session exists but no OTP has been verified against it.")
...
def _session_card(verification_id: str, card_id: str) -> dict:
    ...
    if card["customer_id"] != entry["customer_id"]:
        raise _fail(403, "CARD_NOT_IN_SESSION",
                    "That card belongs to a different customer.")
    return card
...
class IdentifyResponse(BaseModel):
    verification_id: str = Field(
        ..., description=("Issued by this API. Copy this exact string and pass it to "
                          "verify_otp, then on every other call for the rest of the "
                          "conversation."),
        examples=["SES4B7C1E90A2D6"])
    otp_sent: bool
...
class VerifyOtpResponse(BaseModel):
    ...
    reason: Optional[str] = Field(
        None, description="Machine readable reason when verified is false")
...
class UpdateCardRequest(BaseModel):
    ...
    verification_id: str = Field(
        ..., description=("Copy the verification_id string exactly as it appeared in "
                          "the identify_customer response. Do not construct it or build "
                          "it from any other value."),
        examples=["SES4B7C1E90A2D6"])
...
class TicketRequest(BaseModel):
    ...
    address: str = Field(..., examples=["1 Example Street, City, State 000000"])
    issue: str = Field(..., description="What the customer is reporting",
                       examples=["Does not recognise a transaction on the statement"])
...
def identify_customer(body: IdentifyRequest):
    ...
    return IdentifyResponse(
        ...
        message=f"A one time code has been sent to the number ending {masked[-4:]}.")
...
@app.post("/auth/verify-otp", response_model=VerifyOtpResponse, operation_id="verify_otp",
          ...
          description=(
              ...
              "that customer's data for 15 minutes. Keep the card_id for the rest of "
...
def verify_otp(body: VerifyOtpRequest):
    ...
    if not entry:
        ...
        return VerifyOtpResponse(
            verified=False, verification_id=given, reason="VERIFICATION_ID_NOT_ISSUED",
            message=(f"'{given[:40]}' was not issued by this service. Every "
                     f"verification_id is generated by identify_customer and cannot be "
                     f"derived from a mobile number or any other value."))
    ...
    if _now() > entry["otp_expires_at"]:
        ...
        return VerifyOtpResponse(
            verified=False, verification_id=body.verification_id, reason="OTP_EXPIRED",
            message=f"This code expired after {OTP_MINUTES} minutes.")
    ...
    if body.otp != entry["otp"] or entry["customer_id"] is None:
        ...
        if entry["attempts_left"] <= 0:
            ...
            return VerifyOtpResponse(
                ...
                attempts_remaining=0, reason="OTP_ATTEMPTS_EXHAUSTED",
                message=f"Closed after {OTP_ATTEMPTS} incorrect codes.")
        return VerifyOtpResponse(
            verified=False, verification_id=body.verification_id,
            attempts_remaining=entry["attempts_left"], reason="OTP_INCORRECT",
            message=(f"That code is not correct. "
                     f"{entry['attempts_left']} attempts remaining."))
...
def get_customer_address(customer_id: str,
    ...
    if not cust:
        raise _fail(404, "CUSTOMER_UNKNOWN",
                    "No customer exists with that customer_id.")
    entry = _session(verification_id)
    if not entry.get("bypass") and cust["customer_id"] != entry["customer_id"]:
        raise _fail(403, "CUSTOMER_NOT_IN_SESSION",
                    "That customer is not the one verified in this session.")
...
@app.get("/cards/{card_id}", operation_id="get_card", tags=["Reads"],
         ...
         description=(
             "Use whenever the customer asks about their own card: its state, what they "
             "have spent, how much is left, the limits and features currently set on it, "
             "whether their PIN has been changed, their name and address on record, or "
             "their recent transactions. One call "
             "returns all of it, so a second read is not needed to answer a question "
             "about this customer. Read only. Call it again after any change rather than "
             "repeating figures from earlier in the conversation.\n\n"
             "This returns the customer's own values only. It does not explain the "
             "bank's rules. Fees, interest rates, how EMI works, delivery timelines, the "
             "maximum a limit may be set to, what a feature controls, dispute windows, "
             "and why a payment is refused in general all come from the policy lookup "
             "skill, not from here. The emi_options field lists the tenures and rates "
             "available and nothing more; any question about how EMI works needs the "
             "policy lookup skill."))
def get_card(card_id: str, verification_id: Optional[str] = Query(
    ...
    used = _used(card)
    cyc = _cycle()
    cust = CUSTOMERS[card["customer_id"]]
    return {
        "card_id": card["card_id"], "customer_id": card["customer_id"],
        "customer": {"name": cust["name"], "address": cust["address"],
...
def update_card(card_id: str, body: UpdateCardRequest):
    ...
    if card["state"] == CardState.blocked:
        raise _fail(409, "CARD_BLOCKED", "This card is blocked and cannot be changed.")
    changed = {}
    if body.limits:
    ...
    if body.features:
        for field, value in body.features.model_dump(exclude_none=True).items():
            ...
            changed[field] = value
    if not changed:
        raise _fail(400, "NO_CHANGES", "No limits or features were supplied.")
...
@app.post("/cards/{card_id}/replacement", operation_id="request_replacement",
          ...
          description=(
              ...
              "a reference the customer can quote. The fee must be stated to the "
              "customer before they confirm, and comes from the policy lookup skill, not "
              "from this response."))
def request_replacement(card_id: str, body: ReplacementRequestBody):
    ...
    if body.reason == ReplacementReason.lost_or_stolen and card["state"] != CardState.blocked:
        raise _fail(409, "REPLACEMENT_REQUIRES_BLOCK",
                    "A lost or stolen card must be blocked before it can be replaced.")
...
def initiate_pin_reset(card_id: str, body: PinResetRequest, request: Request):
    card = _session_card(body.verification_id, card_id)
    if card["state"] == CardState.blocked:
        raise _fail(409, "CARD_BLOCKED",
                    "This card is blocked and its PIN cannot be reset.")
    ...
    return {"request_id": request_id, "card_id": card["card_id"],
            ...
            "message": "A one time PIN change link has been issued."}
...
def raise_ticket(body: TicketRequest):
    ...
    return {**TICKETS[ref],
            "message": f"Ticket {ref} has been raised."}
...
def get_ticket(reference: str,
    ...
    if not ticket or not (entry.get("bypass")
        ...
        raise _fail(404, "TICKET_UNKNOWN",
                    "No ticket with that reference exists for this customer.")
    return ticket
